...
-- The statement must show the total number of bookings placed on 
-- each of the printed dates using the GROUP BY BookingDate.
SELECT
    BookingDate,
    COUNT(BookingDate)
FROM
...
-- the number of guests for bookings made in the restaurant before 2021-11-13 and where 
-- number of guests is larger than 3.
CREATE VIEW
    BookingsView AS
SELECT
    BookingID,
...
-- Create a stored procedure called 'GetBookingsData'. The procedure must contain 
-- one date parameter called "InputDate". This parameter retrieves all data from 
-- the Bookings table based on the user input of the date.
