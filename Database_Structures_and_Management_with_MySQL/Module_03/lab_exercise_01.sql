...
-- Task 2: Write a SQL SELECT query using an appropriate date function and a 
-- format string to find the name of the weekday on which M&G’s orders are to be delivered.
SELECT
...
SELECT
    ROUND(Cost * 0.05, 2) AS HandlingCost
FROM
    mg_orders;

-- Task 4: Review the query that you wrote in the second task. Use an appropriate comparison function to filter 
-- out the records that do not have a NULL value in the delivery date column.
SELECT
    DATE_FORMAT(DeliveryDate, '%W')
FROM
    mg_orders
