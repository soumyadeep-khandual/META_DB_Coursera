...
-- Task 1: Use the MySQL CEIL function to express the cost after the 
-- discount in the form of the smallest integer as follows:
...
SELECT ClientID, OrderID, CEIL(Cost* 0.95) AS afterDiscount 
FROM client_orders
WHERE ItemID = 4;


-- Task 2: Format the afterDiscount column value from the earlier result for 5% discount in
--  '#,###,###.##' format rounded to 2 decimal places using the FORMAT function.
SELECT ClientID, OrderID, FORMAT(ROUND(Cost* 0.95, 2), 2) AS afterDiscount 
FROM client_orders
WHERE ItemID = 4;
...
-- been delivered. The DeliveryDate column has a NULL value for orders not yet delivered. 
...
-- Task 5: Generate data required for the report by retrieving a list of M&G 
-- orders yet to be delivered. These orders have an 'In Progress' status using the NULLIF function.
